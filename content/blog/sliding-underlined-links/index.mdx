...
lead: Tired of boring links? Bring them to life with these CSS-only underline transitions
date: '2020-09-08'
tags: css, webdev
---
...
With the gradient set, it will make the entire anchor's background to the color/gradient we specify, so we'll need to change it's size and position to make it look like an underline.
...
There's no hover-off psuedo-class in CSS, unfortunately, but there are some [CSS tricks to mimic it for certain effects](https://css-tricks.com/different-transitions-for-hover-on-hover-off/).
...
If you do want an underline to always be present, you can still use the first four effects. The only difference is we need to add an an additional underline using the same `linear-gradient` effect. Note that for the slide to middle effect, we'll have to do add a 3rd `linear-gradient` since that effect already uses 2 of them.
...
For making it this far, I have a little treat for you. I saw this effect making the rounds on Twitter and and it's actually what inspired me to write this post.
